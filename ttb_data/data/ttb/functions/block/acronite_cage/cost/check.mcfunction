...
# run calculations
execute as @e[type=armor_stand,tag=ttb_tempite_stone_nearest,limit=1] at @s run function ttb:block/acronite_cage/cost/modify_tempite

# trigger if had enough tempite
execute unless score tempite_amnt_temp ttb_data matches ..-1 run function ttb:block/acronite_cage/triggered
